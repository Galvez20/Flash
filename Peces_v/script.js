...
tl.to(".felicitaciones",{duration:0,opacity:0, scale:0,x:120, y:-200}); //pocision inicial de mensaje
...
document.getElementById("reinicio").addEventListener("click",()=>{ //activa una funcion tras precionar el boton "Reinicio"
  if(bandera_ayuda==0 || bandera_ayuda ==2){
    ...
    tl.to(".figNull",{duration:0, fill:"#00bfbf"}); //todas las figuras inamovibles regresan a tener su color principal
...
document.getElementById("ayuda").addEventListener("click",()=>{ //activa una funcion tras precionar el boton "A"
  if(bandera_ayuda==0){
    ...
    tl.to(".figAyuda",{duration:0, x:0, y:0, opacity:1, scale:1}) //la figura de ayuda (la que se mueve sola) comienza su reccorio
      .to(".figAyuda",{duration:1.2, rotation:135,x:100})
      .to(".figAyuda",{duration:1, x:538})
    ...
    tl.to(".figNull",{duration:0,fill:"#00bfbf"});
...
Draggable.create("#BigTriangle1", {            
  ...
  onDrag:function(e){                       //funcion que es llamada tras detecta un click del mouse a la figura
    tl.to(this.target, {duration: 0, rotation:135}); //El triangulo gira 45 grados
  },
  onRelease:function(e){   // funcion que es llamada cuando el mouse deja de precionar el elemento
    if (this.hitTest("#BigTriangleNull1", overlapThreshold)){  //si detecta que esta sobre su espacio correspondiente
      ...
      tl.to("#BigTriangleNull1", {duration: 0, fill:"#0bed00"}); //El triangulo gris pasa a tener un color
...
Draggable.create("#BigTriangle2",{
  ...
  onRelease:function(e){
    if(this.hitTest("#BigTriangleNull2", overlapThreshold)){
      ...
      tl.to("#BigTriangleNull2", {duration: 0, fill:"#0090ff"}); 
      conteo++;
      ganar();
    }else {
...
Draggable.create("#SmallTriangle1",{
  type:"x,y",
  onDrag:function(e){
    tl.to(this.target, {duration:0, rotation:180})
  },
  onRelease:function(e){
